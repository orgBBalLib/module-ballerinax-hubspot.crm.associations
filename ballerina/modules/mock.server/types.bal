...
# Association data with labels linking source object to multiple targets
...
public type MockLabelsBetweenObjectPairCreated record {|
    *http:Created;
...
# Pagination object for navigating forward through results
...
public type MockBatchResponsePublicAssociationMultiWithLabelOk record {|
    *http:Ok;
    BatchResponsePublicAssociationMultiWithLabel body;
|};

# Request object for creating associations with specified types
...
public type BatchResponseLabelsBetweenObjectPair record {
    ...
    record {|string...;|} links?;
    # Array of labeled associations between object pairs
...
public type MockBatchResponsePublicAssociationMultiWithLabelWithErrorsMultiStatus record {|
    *http:MultiStatus;
...
# Batch response containing association results with labels, including any errors encountered
public type BatchResponsePublicAssociationMultiWithLabelWithErrors record {
    ...
    string startedAt;
    # Map of related resource links for pagination and navigation
    record {|string...;|} links?;
...
# Represents a default association between two CRM objects with specification
...
public type MockBatchResponseLabelsBetweenObjectPairWithErrorsMultiStatus record {|
    *http:MultiStatus;
    BatchResponseLabelsBetweenObjectPairWithErrors body;
|};

# Request payload containing multiple association entries to create in batch
...
public type MockBatchResponsePublicDefaultAssociationOk record {|
    *http:Ok;
...
# Request payload for fetching multiple associations in a batch operation
...
# Response containing batch operation results with labeled associations and status
...
# Response containing labeled associations between object pairs with error details
public type BatchResponseLabelsBetweenObjectPairWithErrors record {
    # Timestamp when the batch processing completed in ISO 8601 format
    string completedAt;
    # Total count of errors encountered during batch processing
    int:Signed32 numErrors?;
    # Timestamp when the batch request was submitted in ISO 8601 format
    string requestedAt?;
    # Timestamp when the batch processing started in ISO 8601 format
    string startedAt;
    # Map of related resource links for additional context
    record {|string...;|} links?;
    # Array of successfully retrieved label associations between object pairs
    LabelsBetweenObjectPair[] results;
    # Array of errors that occurred during batch processing
    StandardError[] errors?;
    # The current processing status of the batch operation
    "CANCELED"|"COMPLETE"|"PENDING"|"PROCESSING" status;
};

public type MockAnydataDefault record {|
...
# Paginated response containing associated objects with their labels
...
public type MockReportCreationResponseOk record {|
    *http:Ok;
...
# Request payload for fetching associations for a specific object in batch
