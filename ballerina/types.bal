...
# Response object containing report creation details including user info and enqueue time.
public type ReportCreationResponse record {
    # Email of the user
    string userEmail;
    # ID of the user
    int:Signed32 userId;
    # Represents a date-time value with timezone and date-only configuration.
    DateTime enqueueTime;
};

# Represents a multi-association between objects with label information.
...
# Request object for creating an association between two objects with types.
...
# Batch response containing labeled associations between object pairs with status.
...
# Batch request container for fetching multiple association records.
...
# Paginated response containing multiple associated objects with labels.
...
# Request object for fetching associations in batch by object ID.
