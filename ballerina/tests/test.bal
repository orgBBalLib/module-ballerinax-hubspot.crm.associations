...
isolated function testRemoveAssociationBetweenObject() returns error? {
    BatchResponseVoid|error response = hubspotAssociations->/associations/[FROM_OBJECT_TYPE]/[TO_OBJECT_TYPE]/batch/archive.post(
        payload = {
            inputs: [
                {
                    'from: {
    ...
    test:assertTrue(response is BatchResponseVoid, msg = "Expected BatchResponseVoid but got error");
}
...
isolated function testDeleteSpecificLables() returns error? {
    BatchResponseVoid|error response = hubspotAssociations->/associations/[FROM_OBJECT_TYPE]/[TO_OBJECT_TYPE]/batch/labels/archive.post(
        payload = {
            inputs: [
                {
                    types: [
