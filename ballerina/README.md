...
[HubSpot](https://www.hubspot.com/) is a cloud-based customer relationship management (CRM) platform that helps businesses grow better by providing tools for marketing, sales, customer service, and content management in one unified ecosystem.
...
> **Tip:** You must copy and store this key somewhere safe. It won't be visible again without clicking "Show token," and for security reasons, you should treat it as a sensitive credential that should never be shared publicly.
## Quickstart
...
#### Create associations between records using default association type

```ballerina
public function main() returns error? {
    hscrmassoc:BatchInputPublicDefaultAssociationMultiPost payload = {
        inputs: [
            {
                'from: {
                    id: "12345"
    ...
    hscrmassoc:BatchResponsePublicDefaultAssociation response = check hscrmassocClient->/associations/["contacts"]/["companies"]/batch/associate/default.post(payload);
}
```
...
2. [Enterprise onboarding associations](https://github.com/ballerina-platform/module-ballerinax-hubspot.crm.associations/tree/main/examples/enterprise-onboarding-associations) - Illustrates managing associations for enterprise customer onboarding workflows.
