...
2. For full API access to CRM associations, you may need a Professional or Enterprise plan, as some API features and rate limits vary based on your subscription tier. Free and Starter plans have limited API access.
...
> **Tip:** You must copy and store this key somewhere safe. It won't be visible again without clicking "Show token," and you should treat it as a sensitive credential for security reasons.
## Quickstart
...
public function main() returns error? {
    hscrmassociations:BatchInputPublicDefaultAssociationMultiPost batchRequest = {
        inputs: [
            {
                'from: {
                    id: "12345"
    ...
    hscrmassociations:BatchResponsePublicDefaultAssociation response = check hscrmassociationsClient->/associations/["contacts"]/["companies"]/batch/associate/default.post(batchRequest);
}
```
