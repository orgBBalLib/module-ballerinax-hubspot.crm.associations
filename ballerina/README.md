...
[HubSpot](https://www.hubspot.com/) is a cloud-based customer relationship management (CRM) platform that helps businesses manage their marketing, sales, customer service, and content operations through a unified suite of tools designed to grow better.
...
2. To access the CRM Associations API, you need a HubSpot account with a Professional or Enterprise plan subscription. Some API features may be available on free accounts, but full associations functionality requires a paid tier.
...
6. Navigate to the **Scopes** tab and select the required scopes for CRM Associations. Enable `crm.objects.contacts.read`, `crm.objects.contacts.write`, `crm.objects.companies.read`, `crm.objects.companies.write`, and other relevant object scopes based on your needs.
...
```toml
token = "<Your_HubSpot_Access_Token>"
```
...
public function main() returns error? {
    crmassociations:BatchInputPublicDefaultAssociationMultiPost payload = {
        inputs: [
            {
                'from: {
                    id: "12345"
    ...
    crmassociations:BatchResponsePublicDefaultAssociation response = check crmAssociationsClient->/associations/["contacts"]/["companies"]/batch/associate/default.post(payload);
}
```
