...
This example demonstrates how to manage associations between HubSpot CRM objects using the HubSpot CRM Associations connector. The script creates default and labeled associations between deals and companies, reads the associations, and then deletes them both specifically and completely.
...
2. **Configuration**
   ...
   > **Note:** Before running this example, ensure you have valid HubSpot object IDs. Update the `FROM_OBJECT_ID` and `TO_OBJECT_ID` constants in `main.bal` with actual deal and company IDs from your HubSpot account.
