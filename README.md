...
[HubSpot](https://www.hubspot.com/) is a cloud-based customer relationship management (CRM) platform that helps businesses manage their marketing, sales, customer service, and content management through an integrated suite of tools designed to drive growth and build lasting customer relationships.
...
2. If you are using HubSpot's legacy API keys, note that API key access requires a Professional or Enterprise plan. However, for private apps and OAuth-based access tokens (recommended), all HubSpot accounts including free tiers can create private apps to generate access tokens.
...
6. Navigate to the **Scopes** tab and select the required scopes for CRM associations (e.g., `crm.objects.contacts.read`, `crm.objects.contacts.write`, and other relevant CRM scopes).
