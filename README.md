...
[HubSpot](https://www.hubspot.com/) is a cloud-based customer relationship management (CRM) platform that helps businesses grow by providing tools for marketing, sales, customer service, and content management, all unified on a single platform.

The `ballerinax/hubspot.crm.associations` package offers APIs to connect and interact with [HubSpot CRM Associations API](https://developers.hubspot.com/docs/reference/api/crm/associations) endpoints, specifically based on [HubSpot CRM Associations API v4](https://developers.hubspot.com/docs/reference/api/crm/associations/association-details).
## Setup guide
...
2. Note that while HubSpot offers a free CRM tier, some API features and higher rate limits may require a Professional or Enterprise subscription plan depending on your usage requirements.
...
6. Navigate to the **Scopes** tab and select the required scopes for CRM associations (e.g., `crm.objects.contacts.read`, `crm.objects.contacts.write`, `crm.objects.companies.read`, `crm.objects.companies.write`, and other relevant object scopes).

7. Click **Create app** in the top right corner, then review the details and click **Continue creating**.

8. Your access token will be displayed. Copy this token to use with the connector.
...
2. [Create read associations](https://github.com/ballerina-platform/module-ballerinax-hubspot.crm.associations/tree/main/examples/create-read-associations) - Illustrates creating associations and retrieving association details from HubSpot CRM.
3. [Merger association management](https://github.com/ballerina-platform/module-ballerinax-hubspot.crm.associations/tree/main/examples/merger-association-management) - Shows how to manage associations during record merging operations in HubSpot CRM.
